...
## 7. Mode 2048 — tenté puis retiré

Une variante 2048 pour réviser une table (tuiles en expression `4×3`
tant que non fusionnées, résultat `12` une fois fusionnées, plafond
d'affichage à ×10) a été implémentée, testée et déployée, puis
entièrement retirée à la demande de l'utilisateur : "le concept ne
fonctionne pas" en pratique une fois essayé, malgré une conception
qui semblait cohérente sur le papier (tuiles jamais bloquées, chrono
5-10 min, objectif "table maîtrisée"). Code supprimé (`game/game2048.ts`,
`components/Mode2048*.tsx`, CSS associé) plutôt que laissé désactivé,
pour ne pas garder de code mort. À ne pas re-proposer sans revoir le
concept de fond, pas juste les détails d'implémentation.
